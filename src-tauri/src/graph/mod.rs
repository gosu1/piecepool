//! 그래프 도메인 — node-compat 매트릭스·중복/self-loop 판정·노드 조립·관계 검증.
//! 규약: docs/10-contracts/relation-types.md (enum 은 models 미러, 매트릭스는 로직).
//! commands/graph.rs 는 이 모듈의 얇은 IPC 위임이다.
...
/// 검증 실패 오류 — 프론트에는 "[relation_invalid] …" 문자열로 나간다.
fn invalid(msg: impl Into<String>) -> AppError {
    AppError {
        kind: "relation_invalid".into(),
        message: msg.into(),
    }
}
...
pub fn read_relations(space: &str) -> Result<Vec<Relation>> {
...
pub(crate) fn write_relations(space: &str, rels: &[Relation]) -> Result<usize> {
    let path = storage::space_subdir(space, "relations").join("relations.json");
...
pub fn load_graph(space: &str, space_id: &str) -> Result<GraphData> {
    ...
    let pages = storage::list_parsed(space, "wiki", |f, md| {
        frontmatter::md_to_wiki(space_id, f, md)
            .ok()
            .map(|p| (f.to_string(), p))
    })?;
    let mut nodes = vec![];
...
pub fn append_validated(space: &str, relations: Vec<Relation>) -> Result<usize> {
    ...
    for r in relations {
        ...
        if !(0.0..=1.0).contains(&r.strength) || !(0.0..=1.0).contains(&r.confidence) {
            return Err(invalid(format!(
                "strength/confidence 는 0~1 이어야 함: {edge}"
            )));
        }
        if r.relation_type == RelationType::ReviewNeeded {
            return Err(invalid(
                "review_needed 는 사용자만 지정 가능(자동 부여 금지)",
            ));
        }
        if !compat(
        ...
        if r.evidence.is_empty() {
            return Err(invalid(format!("모든 관계는 evidence ≥ 1: {edge}")));
        }
...
///
/// [`append_validated`] 의 **거울상**이다. 저쪽은 review_needed 만 거부하고,
/// 이쪽은 review_needed 만 허용한다. 두 함수가 나뉘어 있다는 사실 자체가
...
pub fn mark_review(
    ...
    if node_kind(&concept_id) != NodeKind::Concept {
        return Err(invalid(format!(
            "review_needed 는 Concept 에만 붙는다: {concept_id}"
        )));
    }
    if source_id.trim().is_empty() {
        return Err(invalid("source_id 필수"));
    }
    let quotes: Vec<String> = quotes
        ...
        .collect();
    if quotes.is_empty() {
        return Err(invalid("모든 관계는 evidence ≥ 1: 설명 시도가 곧 근거다"));
    }
...
pub fn unmark_review(space: &str, concept_id: &str) -> Result<usize> {
