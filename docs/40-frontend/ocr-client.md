...
이미지는 별도의 업로드 화면이 아니라 연속 note 편집 화면에 **인라인으로 첨부**된다 ([`scope-mvp.md`](../00-overview/scope-mvp.md) §2.3). 첨부 시 사용자가 그 이미지에 대한 설명을 직접 입력할 수 있으며, 이 사용자 설명은 §4에서 별도 블록으로 archive에 보존된다. (별도 "dropzone" 컴포넌트는 두지 않음 — 첨부 UI는 `markdown-editor.md`(#21) 소관)
