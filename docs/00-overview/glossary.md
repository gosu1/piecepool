...
| 용어 | 정의 |
|---|---|
| **파인만 (Feynman)** | 사용자가 개념을 자기 말로 설명하면 LLM이 정답을 주지 않고 그 설명의 구멍 하나만 짚어 되묻는다. 이해 판정은 오직 사용자가 한다 (LLM은 채점하지 않는다) |
...
| 용어 | 정의 |
|---|---|
| **디렉토리 바 (Directory Bar)** | 앱 좌측 파일 트리 사이드바. 지식 스페이스 → wiki/source → 파일 트리. GitHub(이슈·PR·문서)에서 이 영역의 공식 명칭 |

상세 구조·인터랙션: [`50-design/directory-bar`](../50-design/directory-bar.md).
...
- §11 (UI 영역 명칭)은 "디렉토리 바" 명명 확정과 함께 추가됐다.
