...
PiecePool 제품 모델. 모든 역할이 읽는다. **단일 tier — OpenAI premium-only**.
...
PiecePool은 **OpenAI**(GPT)를 LLM provider로 사용한다. Free/Premium 구분 없는 단일 tier다.
...
- **2026-06-30**: 3-provider hybrid + freemium 폐기 → **OpenAI 단일 tier** (premium-only). local(llama.cpp/Gemma)·Gemini 제거. Free 플랜 삭제, "Premium 기능"→기본 기능으로 통합.
