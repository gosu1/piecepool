...
- 넷째 줄 → 본문에 svg 가 그림으로 보인다. **이건 수정 전에도 보였다** — 렌더는 `FilePreview` 의 자체 `MIME` 맵을 타서 원래 정상이었다. 회귀만 확인하는 항목이고, 실제로 깨져 있던 것은 노트 삭제 시 svg 가 디스크에 남는 것과 svg 전용 노트의 sourceRefs 가 날아가는 것이다(설계 문서 §1.2)
...
- `[[lecture.pdf]]` 클릭: 수정 전(회색 점선 + "연결된 위키가 아직 없어요" 툴팁, 눌러도 무반응) / 수정 후(파란 링크 → `lecture.pdf` 원본 탭, 1/12 쪽)
- `[[lecture.pdf#page=200]]` 클릭 후: 툴바 아래 "요청한 200쪽이 범위를 벗어남(총 12쪽) — 1쪽을 표시합니다." 배너

svg 는 비포·애프터 쌍이 없다. 수정 전에도 그림으로 보였기 때문이다 — 첨부하지 말고 PR 본문에 그 사실을 적는다.
