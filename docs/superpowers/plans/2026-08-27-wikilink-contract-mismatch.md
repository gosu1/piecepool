...
인박스 탭에서 페이지 수가 열 쪽 안팎인 PDF 하나(`강의자료.pdf` 로 가정)와 svg 파일 하나(`그림.svg` 로 가정)를 업로드해 `sources/original-files/` 에 넣는다.

- [ ] **Step 3: 노트에 세 줄을 넣는다**

위키 문서 하나를 편집 모드로 열고 본문에 다음 세 줄을 넣은 뒤 저장한다.
...
- 첫 줄 클릭 → `Original` 탭이 새로 열리고 PDF 1쪽이 보인다
- 둘째 줄 클릭 → **같은 탭**이 유지되고, 툴바 아래에 "요청한 200쪽이 범위를 벗어남(총 N쪽) — 1쪽을 표시합니다." 가 뜬다
- 셋째 줄 → 본문에 svg 가 그림으로 보인다 (수정 전에는 안 보였다)

세 번째 확인을 위해 인박스 화면의 위키 패널에서도 첫 줄 클릭이 원본 탭을 여는지 함께 본다.

- [ ] **Step 5: 비포·애프터 스크린샷을 찍는다**

CLAUDE.md 의 협업 규칙상 UI 변경 PR 은 비포·애프터 스크린샷이 필수다. 에이전트는 앱 화면을 캡처할 수 없으므로 **사용자에게 요청한다.** 필요한 것은 두 쌍이다.

- `[[강의자료.pdf]]` 클릭: 수정 전("연결된 위키가 아직 없어요" 툴팁) / 수정 후(원본 탭)
- `![[그림.svg]]`: 수정 전(안 보임) / 수정 후(그림)
