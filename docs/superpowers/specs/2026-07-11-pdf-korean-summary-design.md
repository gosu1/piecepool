...
사용자는 유니브AI처럼 영어 논문이 **한국어로 번역·요약된 본문**으로 채워지길 원했다.
유니브AI와의 차별점: 결과가 **사용자가 직접 편집 가능한 마크다운**이어야 한다.
...
3. **수식 렌더**: 노션식 — 평소 렌더, 커서 접근/클릭 시 원문 `$...$` 노출.
4. **콜아웃 접기**: 옵시디언식 — 편집기 안에서도 셰브론으로 접기(CM6 내장 codeFolding), 읽기 모드는 `<details>`.
