...
| 파인만 `src/llm/feynman.ts:65` | `"in Korean 존댓말"` → lang 분기: ko는 존댓말 유지, en은 "polite, encouraging English" |
| 간극질문 `src/llm/gaps.ts:122` | `"in Korean"` → directive. Liner 검색 쿼리 템플릿(`gaps.ts:76` `"핵심 개념 정의"`)도 en이면 영어 쿼리 |
| 핵심주제 `src/llm/coretopics.ts:81` | `reason` 언어만 lang 분기. system 본문은 한국어 유지(판별 품질 안 건드림). 캐시 키 `core-topics:<sourceId>`에 `:<lang>` suffix — 언어 변경 시 재판정. **⚠ 의존성**: 이 파일은 2026-07-12 현재 main 미도달 — PR #188이 `feat/feynman-sections`로 머지됐고(#186이 main에 먼저 머지된 뒤), 게이트를 main으로 올리는 후속 머지가 선행돼야 한다. 구현 시점에 main에 없으면 이 항목만 후속 PR로 분리 |
| PDF 요약 `src/llm/pdfsummary.ts` | "영어→한국어"가 정체성 → "→ 설정 언어"로 일반화. en이면 번역 없이 영어 요약 노트. 병기 규칙(`:52` 「한국어 번역(영어 원어)」)은 ko 전용 유지 |
| 합성 `src/llm/synthesize.ts:43` | 규칙 7 "본문은 한국어. 용어·식별자는 파편의 원문 표기를 따른다" → directive로 대체 (원문 표기 규칙은 directive §4-1에 포함됨) |
| OCR `src/llm/ocr.ts` | 출력 헤딩 lang 분기: `## 원문/구조/요약` ↔ `## Original/Structure/Summary`. 요약 블록 언어도 directive를 따름 |
| 휴리스틱 폴백 `src/llm/generate.ts:174,206` | 키 없는 오프라인 모드의 한국어 고정 문구("…하위 주제다. (휴리스틱)", "`${title} 개념 정리`")를 2벌로 분기 |
| UI 문구 `src/app/panes/InboxSection.tsx:237,775` | "PDF를 한국어로 요약해요/요약하고 있어요" → 설정 언어명 보간 (문구 거짓말 방지) |
...
- `docs/10-contracts/llm-output-schema.md:67`: `// 한국어 1-2문장` → `// 사용자 언어 설정을 따름, 1-2문장`
