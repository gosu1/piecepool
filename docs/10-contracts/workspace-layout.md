...
PiecePool Workspace/
  ...
  queries/                      # 쿼리바 대화 기록 (숨기지 않음 — §3.10)
    sessions/                   # 대화 1건 = JSON 파일 1개
      2026-08-23-브랜치-규칙.json
    commands/                   # 사용자 정의 슬래시 명령 (자리 예약, 현재 미사용)
    config.json                 # 쿼리바 설정
...
### 3.10 `queries/` (Workspace 루트)

쿼리바가 쓰는 폴더. 지식 영역 폴더와 같은 층위인 Workspace 루트에 둔다.

| 경로 | 내용 |
|---|---|
| `sessions/` | 대화 기록. 대화 1건 = JSON 파일 1개 |
| `commands/` | 사용자가 추가하는 슬래시 명령. 자리만 예약하고 현재 코드는 읽지 않는다 |
| `config.json` | 쿼리바 설정 |

**지식 영역 폴더 안이 아니라 루트에 두는 이유**: 쿼리바는 모든 지식 영역을 가로질러 읽는다. 한 대화가 두 영역의 위키를 함께 인용할 수 있으므로 어느 영역에도 속하지 않는다.

**숨기지 않는다.** `.config/`(§3.1)와 달리 대화 기록은 앱이 쓰는 설정이 아니라 사용자가 만든 지식이다. 탐색기로 열어보고, 백업하고, 다른 기기로 옮길 수 있어야 한다 — §3.1 의 "숨김 대상은 앱이 쓰는 설정이지 사용자가 쓴 지식이 아니다"를 그대로 따른다.

`queries` 는 지식 영역 폴더가 쓸 수 없는 예약 이름이다(§4).
...
- `queries` 도 예약어다 — 쿼리바 폴더(§3.10)와 충돌하면 안 된다. 충돌 시 `queries 2`.
...
- 2026-08-23: Workspace 루트에 `queries/` 추가(§3.10) — 쿼리바 대화 기록. 예약 이름에 `queries` 추가(§4).
