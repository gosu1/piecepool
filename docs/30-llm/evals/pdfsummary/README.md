...
**케이스별 판정은 결과 JSON의 `samples[].judge`에 남는다.** 판정이 성립하면 `{ "verdict": { "hallucination": ..., "hallucinationEvidence": "..." } }`, 실패하면 `{ "error": "judge: HTTP 503 (3회 시도 소진)" }`다. `hallucination`이 `false`인 케이스도 남긴다 — "판정을 받았고 깨끗했다"와 "판정 자체가 없었다"는 다른 사실이다. 지표는 `hallucination`(참 판정 건수)·`judgeFail`(판정 실패 건수)로 그대로고, 이 필드는 **게이트가 깨졌을 때 어느 fixture의 어떤 문장인지 run 파일만 열어서 확인하기 위한 기록**이다(PIE-59). `--dry`에서는 필드 자체가 생기지 않는다. 이 기록은 지금 이 어댑터에만 있다 — judge를 쓰는 다른 어댑터(`synthesize`)는 아직 집계 수치만 남긴다(후속).

judge 호출은 실패 시 **3회까지 시도하고 대기는 5s → 20s**다. `Retry-After`(초)가 오면 **기본 스케줄보다 길 때만 채택**하고 60초로 캡한다 — 기본보다 짧으면 기본 스케줄을 유지한다(짧은 재시도를 연발해 한도만 태우지 않기 위해서다). HTTP-date 형태는 무시하고 기본 스케줄로 간다. **응답이 아예 안 온 경우(30초 타임아웃·네트워크 오류)와 본문이 깨진 경우(200인데 JSON이 아님)도 같은 스케줄로 재시도하며**, 이때 `samples[].judge.error` 값은 `HTTP NNN`이 아니라 원인 문자열이다(`judge: The operation was aborted due to timeout (3회 시도 소진)` · `judge: bad body (...) (3회 시도 소진)`). 시도 횟수를 안 늘리는 이유는 심판 무료 한도(20콜/일)다 — fixture 4종 × 최대 3콜 = 최악 12콜이어야 러너 1회 실행이 재시도까지 포함해 완주한다. `npm run eval:all`은 judge를 함께 쓰는 `synthesize`(fixture 1종, 최대 3콜)가 더해져 **최악 15콜**이다.
...
**judge 가용성이 이틀째 측정을 가른다.** 2026-08-04는 심판(`gemini-3.5-flash`)이 종일 `HTTP 503`("high demand") → `HTTP 429`(무료 티어 일일 한도 20)로 `judgeFail 4` — 판정 표본 0이었다. 2026-08-05 이 실행에서는 4건 중 2건이 판정에 성립했고 **둘 다 환각으로 판정됐다.** 러너가 케이스별 판정 결과를 결과 JSON에 남기지 않아 어느 fixture인지는 기록되지 않는다(PIE-59에서 해소 — 이후 실행부터 `samples[].judge`에 남는다).
...
> 운영 메모: 당시 `judge.ts`의 백오프가 250·500ms로 짧아 503 고수요 구간에서 재시도가 그대로 소진됐다 — PIE-59에서 5s → 20s(`Retry-After`가 기본보다 길 때만 채택, 60초 캡)로 늘렸고 시도 횟수는 3회 그대로다. 예산 산술은 그대로다: fixture 4종 × 최대 3콜 = 최악 12콜이라 무료 티어 일일 한도(20)로는 하루 한 번 완주가 한계다. **eval을 동시에 두 개 돌리면 예산이 이중으로 탄다(2026-08-05 실증)** — 심판 예산 확인 후 직렬로만 돌릴 것.
...
| 2026-08-07 | 케이스별 judge 판정 기록(`samples[].judge`) + judge 백오프 250·500ms → 5s·20s(`Retry-After` 캡 60s) — 게이트·임계값 무변경 | 집계 수치만으로는 어느 fixture의 어떤 문장이 걸렸는지 되짚을 수 없었다(PIE-59). 백오프는 PIE-55 실측(503 고수요 구간에서 250·500ms 무력)이 근거. 시도 횟수는 예산(20콜/일, 4×3=12) 때문에 3회 유지 |
