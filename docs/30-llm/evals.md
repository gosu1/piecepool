...
- **케이스별 judge 판정은 결과 JSON의 `samples[].judge`에 남는다 — 현재 `pdfsummary` 어댑터 한정이다.** 판정이 성립하면 `{ "verdict": ... }`(어댑터가 쓰는 판정 스키마 그대로 — 환각 여부와 근거 문장), 실패하면 `{ "error": "judge: HTTP 503 (3회 시도 소진)" }`처럼 마지막 상태와 시도 횟수가 들어간다. `--dry`에서는 필드 자체가 없다. 집계 수치(`hallucination` 몇 건)만 있으면 게이트가 깨져도 **어느 fixture의 어떤 문장 때문인지 되짚을 수 없다**(PIE-59). judge를 쓰는 다른 어댑터(`synthesize`)는 아직 집계 수치만 남긴다 — 후속.
