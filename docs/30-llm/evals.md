...
- **케이스별 judge 판정은 결과 JSON의 `samples[].judge`에 남는다 — judge를 쓰는 두 어댑터(`pdfsummary`·`synthesize`)가 모두 채운다(PIE-59·PIE-61).** 판정이 성립하면 `{ "verdict": ... }`(어댑터가 쓰는 판정 스키마 그대로 — pdfsummary는 환각 1필드, synthesize는 환각+모순 2필드로 모양이 다르지만 슬롯은 verdict 객체를 통째로 담으므로 가리지 않는다), 실패하면 `{ "error": "judge: HTTP 503 (3회 시도 소진)" }`처럼 마지막 상태와 시도 횟수가 들어간다. `--dry`에서는 필드 자체가 없다. 집계 수치(`hallucination` 몇 건)만 있으면 게이트가 깨져도 **어느 fixture의 어떤 문장 때문인지 되짚을 수 없다**(PIE-59).
- **judge 재시도는 3회 그대로, 대기만 5s → 20s다**(`scripts/evals/judge.ts` `judgeRetryDelayMs`). 250·500ms였던 구 스케줄은 503 고수요 구간을 그대로 관통해 시도가 몇백 ms 안에 소진됐다. 응답에 `Retry-After`(초)가 오면 **기본 스케줄보다 길 때만 채택**하고 60초로 캡한다 — 기본보다 짧으면 기본 스케줄을 유지한다(짧은 재시도를 연발해 한도만 태우지 않기 위해서다). HTTP-date 형태의 `Retry-After`는 시계 차가 그대로 오차가 되므로 무시하고 기본 스케줄로 간다. **응답이 아예 안 온 경우(30초 타임아웃·네트워크 오류)와 본문이 깨진 경우(200인데 JSON이 아님)도 같은 스케줄로 재시도하며**, 이때 `samples[].judge.error`에 남는 값은 `HTTP NNN`이 아니라 원인 문자열이다(`judge: The operation was aborted due to timeout (3회 시도 소진)` · `judge: bad body (...) (3회 시도 소진)`). **시도 횟수를 늘리지 않는 이유는 예산이다** — 심판 무료 한도가 20콜/일이라 fixture 4개 × 최대 3콜 = 최악 12콜이어야 재시도까지 포함해 한 번 완주한다. `npm run eval:all`은 judge를 함께 쓰는 `synthesize`(fixture 1종, 최대 3콜)가 더해져 **최악 15콜**이다.
