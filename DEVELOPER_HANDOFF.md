...
- 과목, 학기, 시험, 프로젝트는 Workspace 분리 기준이 아니라 metadata/filter다.
- 사용자가 입력한 원문은 `archive/`에 실제 `.md` 파일로 저장한다.
- LLM 정리 결과는 `wiki/`에 실제 `.md` 파일로 저장한다.
- Graph View는 MVP 핵심 기능이다.
- Relation은 typed relation이어야 한다.
- Edge 클릭 시 relation type, explanation, evidence가 보여야 한다.
- 실제 LLM 호출을 한다.
- PDF text extraction은 MVP 범위다.
...
계획은 task 단위로 나뉘어 있다. 각 task는 테스트 작성, 실패 확인, 구현, 통과 확인, 커밋 순서로 진행한다.

권장 작업 방식:

1. Task 1부터 순서대로 진행한다.
2. 각 task 완료 후 테스트를 실행한다.
3. task 단위로 커밋한다.
4. Task 10까지 완료하면 최소 vertical slice가 나온다.
5. Task 13까지 완료하면 데모 가능한 Wiki/Graph UI가 나온다.
6. Task 15까지 완료하면 MVP 검증 기준이 갖춰진다.
...
`PIECEPOOL_LLM_MODEL`은 기본값을 둘 수 있다. `OPENAI_API_KEY`가 없으면 LLM import는 실패 메시지와 retry 흐름을 보여줘야 한다.
...
- PDF import가 텍스트를 추출하고 `archive/*.md`를 생성한다.
- 실제 LLM 호출 결과가 `wiki/*.md`와 `relations/relations.json`을 생성한다.
- Markdown editor에서 wiki 파일을 수정하고 저장할 수 있다.
- 앱 재실행 후 저장 내용이 복원된다.
- Graph View가 relation metadata에서 렌더링된다.
- Graph node 클릭 시 연결 문서가 열린다.
- Graph edge 클릭 시 evidence panel이 열린다.
- seed data가 실제 파일과 metadata로 존재한다.
